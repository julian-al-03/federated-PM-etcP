...
class EncryptedNumber:
    ...
    def obfuscate(self):
        r_obf = mpz(randbelow(self.public_key.n)) + 1
        
        # Multiply the original ciphertext by r_obf^n^s % n^(s+1)
        obf_factor = pow_mod(r_obf, self.public_key.n_s, self.public_key.n_s_1)
        self.value = (self.value * obf_factor) % self.public_key.n_s_1
...
class PrivateKeyShare:
    ...
    def decrypt(self, c: EncryptedNumber | int) -> int:
        ...
        """
        # check if c is an EncryptedNumber or an integer
        if isinstance(c, EncryptedNumber):
            return pow(c.value, self.two_delta_s_i, self.public_key.n_s_1)
        return pow(c, self.two_delta_s_i, self.public_key.n_s_1)

    def final_decrypt(self, c_list: list[int], i_list: list[int]) -> any:
...
def keygen(n_bits: int = 64,
    ...
    return public_key, private_key_shares, private_key_ring